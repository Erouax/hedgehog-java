...
import wtf.villain.hedgehog.client.exception.HttpClientErrorException;
		...
		client.newCall(httpRequest).enqueue(new Callback() {
			...
			public void onResponse(@NotNull Call call, @NotNull Response response) {
				...
				if (code >= 200 && code < 300) {
					...
					try {
						String errorBody = body != null ? body.string() : "No response body";
						String errorMessage = String.format("HTTP %d: %s", code, errorBody);

						if (code >= 400 && code < 500) {
							request.responseFuture().ifPresent(future ->
									future.completeExceptionally(new HttpClientErrorException(code, errorMessage, errorBody)));
						} else {
							request.responseFuture().ifPresent(future ->
